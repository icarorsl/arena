...
public:
    ...
    struct TIR { uint32_t table_id=0,group_id=0; std::string name; uint64_t chunk_size=0; uint8_t replication_factor=0; EncryptionAlgo encryption=EncryptionAlgo::NONE; uint32_t max_versions=0; ExpiryGranularity expiry_granularity=ExpiryGranularity::UNSET; };
    std::vector<VIR> list_versions(uint64_t lid);
    std::vector<FIR> list_files(uint32_t gid,uint32_t tid);
    std::vector<TIR> get_tables();
    SR create_table(uint32_t tid,uint32_t gid,const std::string& name);
    bool ping() const;
